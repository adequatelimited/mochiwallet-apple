// iOS-only patch: hide export wallet button and backup section
(function () {
  var MARK_ATTR = "data-ios-hidden-export";
  ...
  function scanAndHide() {
    // 1. Hide export wallet buttons - use specific selectors first
    // Try data attributes and aria labels for more reliable detection
    document.querySelectorAll('[data-testid*="export"], [aria-label*="export" i]').forEach(function (el) {
      hide(el);
    });
    ...
    // 2. Hide "Backup" heading - try class selectors first
    document.querySelectorAll('[class*="backup" i], [data-section="backup"]').forEach(function (el) {
      hide(el);
    });
    
    // Fallback: heading text matching
