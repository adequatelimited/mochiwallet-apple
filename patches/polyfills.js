// iOS WKWebView polyfills for Chrome Extension APIs
// Version: 0.0.18
// Provides compatibility for Chrome extension features in WKWebView environment
//
// This file is CRITICAL for the wallet to function in iOS. It provides:
// 1. Buffer polyfill - Required for Mochimo WOTS+ address generation
// 2. Chrome extension API stubs - runtime, storage, tabs
// 3. iOS native bridge - Communication with Swift code
//
// Without this file, the wallet will fail with errors like "Invalid tag"
// or silently fail to store/retrieve wallet data.

// Polyfill version for debugging
const POLYFILL_VERSION = '0.0.18';

// ============================================================================
// DEBUG MODE - Set to true for verbose logging during development
// When enabled, logs detailed information about Buffer operations and
// Chrome API calls to help diagnose compatibility issues.
// ============================================================================
...
// BUFFER POLYFILL
// ============================================================================
// The Mochimo wallet uses Buffer for binary data operations, especially for:
// - WOTS+ signature generation (hex string → byte array conversion)
// - Address encoding/decoding
// - Transaction serialization
//
// WKWebView doesn't have Node.js Buffer, so we provide a compatible
// implementation using Uint8Array with additional methods.
// ============================================================================
...
if (typeof window.Buffer === 'undefined') {
  window.Buffer = {
     ...
     * @param {string|Uint8Array|ArrayBuffer|Array} data - Input data
     * @param {string} encoding - 'base64', 'hex', 'utf8', or undefined
...
// ============================================================================
// Extends Uint8Array prototype with Buffer-compatible methods for:
// - toString(encoding) - Convert bytes to base64/hex/utf8 strings
// - writeUInt32LE - Write 32-bit integers (transaction amounts)
// - writeBigUInt64LE - Write 64-bit integers (timestamps, large amounts)
// - copy - Copy bytes between arrays
// ============================================================================
...
// ============================================================================
// Chrome extensions expect certain APIs that don't exist in WKWebView.
// These polyfills provide compatible implementations:
// - chrome.runtime - Extension lifecycle and messaging
// - chrome.storage.local - Persistent key-value storage (uses localStorage)
// - chrome.tabs - Tab management (mostly no-ops for single-page app)
// ============================================================================
...
// Mark this as an iOS WKWebView environment so the app knows not to show extension-only features
window.__MOCHI_IOS_WEBVIEW__ = true;

window.chrome.runtime = {
  id: 'ios-webview-mock',
  getURL: function(path) {
    // iOS uses different path structure - resources are in app bundle
    return path;
  },
...
// STORAGE SERIALIZER
// ============================================================================
// Custom serialization for localStorage that preserves binary data types.
// JSON.stringify loses type information for Uint8Array, ArrayBuffer, etc.
// This serializer adds type markers that are restored on deserialization.
//
// Format: { __type: 'Uint8Array', __data: [1, 2, 3, ...] }
// ============================================================================
...
// IOS NATIVE BRIDGE
// ============================================================================
// Provides JavaScript interface to native iOS functionality via
// WKScriptMessageHandler. Messages are sent to Swift handlers:
// - iOSBridge: General actions (device info, haptics, exit)
// - log: Console logging to native
// - toast: Display native toast messages
// ============================================================================

// iOS-specific bridge to native code
window.iOSBridge = {
  showToast: function(message) {
    if (window.webkit && window.webkit.messageHandlers && window.webkit.messageHandlers.toast) {
  ...
  getDeviceInfo: function() {
    if (window.webkit && window.webkit.messageHandlers && window.webkit.messageHandlers.iOSBridge) {
      window.webkit.messageHandlers.iOSBridge.postMessage({ action: 'getDeviceInfo' });
    }
  },
  vibrate: function() {
    if (window.webkit && window.webkit.messageHandlers && window.webkit.messageHandlers.iOSBridge) {
      window.webkit.messageHandlers.iOSBridge.postMessage({ action: 'vibrate' });
    }
  }
};
...
// ============================================================================
// Verify that essential Web Crypto APIs are available. Without these,
// the wallet cannot securely generate keys or encrypt data.
// ============================================================================
...
debugLog('iOS WKWebView polyfills loaded');
